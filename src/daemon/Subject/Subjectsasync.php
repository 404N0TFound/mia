    ...
    public function execute()
    {
        ...
        while (true) {
            ...
            if (!empty($asyncInfo)) {
                ...
                //当天参加活动
                if ($this->task_setting["partake"] === 1 && !$redis->hExists($redis_task_hash_key, 'partake')) {
                    //判断活动id
                ...
                //当天参加消消乐
                if ($this->task_setting["xiaoxiaole"] === 1 && !$redis->hExists($redis_task_hash_key, 'xiaoxiaole')) {
                    $activeService = new Active();
                    $subjectActiveInfo = $activeService->getActiveSubjectBySids([$subjectId],[])["data"];
                    var_dump($subjectActiveInfo);
                    if(!empty($subjectActiveInfo)) {
                        //参加了活动
                        //判断活动id，是否是消消乐
