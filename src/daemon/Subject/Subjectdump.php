...
class Subjectdump extends \FD_Daemon {
    ...
    private function dump_subject($data, $lastId = null) {
        ...
        foreach ($data as $value) {
            ...
            } else if (!empty($extInfo['image'])) {
                $dumpdata['cover_image_width'] = $extInfo['image'][0]['width'];
                $dumpdata['cover_image_height'] = $extInfo['image'][0]['height'];
            } else {
            ...
            if (isset($extInfo['is_material']) && !empty($extInfo['is_material'])) {
                $dumpdata['is_material'] = $extInfo['is_material'];
            }else{
                $dumpdata['is_material'] = null;
            }
