...
class Cleanlivesetting extends \FD_Daemon
{
    public function execute()
    {
        ...
        for ($i = 1; $i <= $end; $i++) {
            $roomList = $liveMod->getLiveRoomList($i, $limit);
            ...
            foreach ($liveList as $live) {
                ...
                //清空
                if(isset($setting['user_num'])) {
                    unset($setting['user_num']);
                    $check = 1;
                }
                if(isset($setting['push_time'])) {
                ...
                //live表增加设置
                if(!empty($trans)) {
                    $liveMod->updateLiveInfo(['id',$live['id']],[['settings',json_encode($trans)]]);echo $live['id']; exit;
                }
            }
        }
    }
