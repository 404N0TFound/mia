...
// 		$url = $this->solrserver . $this->core . '/update?commit=true';
    ...
    public function getHighQualityKoubeiByBrandId($category_id, $brand_id = 0, $page = 1, $category_name)
    {
        ...
        $koubei_list = json_decode($result, true);
        ...
        if(empty($koubei_list)) {
            ...
            // 缓存
            $redis->setex($koubeiListKey, json_encode($koubei_list), 20*60);
        }
        return $koubei_list;
    }
    ...
    * */
    ...
    public function brandList($category_id, $category_name)
    {
        ...
        $brand_ids = json_decode($cache_brand_list, true);
        ...
        if(empty($brand_list)){
            ...
            // 缓存
            $redis->setex($brandListKey, json_encode($brand_ids), 20*60);
        }
        return $brand_ids;
    }
